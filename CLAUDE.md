...
Multi-locale Orthodox Church calendar iOS app (Serbian, Russian, English Old Calendar, English New Calendar). Fully self-contained with bundled JSON data generated by a Python pipeline. Also has a Cloudflare Worker API serving the same data from R2, and an Android port in progress.
...
# Android build
cd /path/to/OrthodoxCalendarAndroid && ./gradlew assembleDebug
```
