...
- Serbian fasting goes through its own function, `spc_fasting_level` — see Fasting Engine. Feast rank plays no part in it; `feast_rank` only relaxes a Russian/OCA fast
...
### Fasting Engine
7 levels: `totalAbstinence`, `dryEating`, `hotNoOil`, `hotWithOil`, `fish`, `fishRoe`, `free`. `compute_fasting(date, pasch, feast_rank, locale)`; `sr` goes to `spc_fasting_level`, every other locale through the Russian/OCA tables.
...
**Russian/OCA**: the period tables plus fixed and movable exceptions that only *relax* a day's rule. Strict feasts impose instead (`strict_feast_level`): the Beheading and the Exaltation are oil on any weekday, Theophany Eve strict on a weekday and oil at a weekend — what pravoslavie.ru and orthocal.info both show.
...
9. **Fasting validation**: SPC fasting is checked in CI against pravoslavno.rs's fasting calendar, 2025–2037 (`validate_spc_fasting.py`). To extend it, scrape more years, re-run `derive_spc_fasting.py`, and check `--holdout` before trusting the new table. The old "93% match" was measured against the month table's "пост" marker, which pravoslavno.rs also prints on Pascha and all of Bright Week — it never compared a level. Russian and English have no such check yet: against pravoslavie.ru and orthocal.info for 2026 they agree on about 82% and 76–78% of days.
